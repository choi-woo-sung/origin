...
함수: 로직을 모듈화 한 코드
==> 실제 사용(호출)하는 곳과 함수가 구현되어있는 부븐을 분리 ==> 유지보수의 편이성을 도모
함수를 사용하지 않을경우
호출하는 부분에 함수코드를 직접 기술해야 하므로, 코드가 길어진다==> 가독성이 나빠진자
...
입력 구분 :
 ...
 . 사용자 정의 함수 : 개발자가 직접정의한 함수 (pl/sql배울때)
 
 
 프로그래밍 언어, 식별이름 부여... 중요한 원칙
 ...
 emp 테이블의 SMITH 사원의 이름은 대문자로 저장되어있음
 
 SELECT*
 FROM emp
 WHERE LOWER(ename)=  'smith';이런식으로 작성하면 안된다 ->행 개수만큼 실행.
 
 WHERE ename = UPPER ('smith'); 두가지 방식중에는 위에보다는 아래방식이 올바른 방시이다
 
 WHERE ename ='smith'; 테이블에는 데이터값이 대문자로 저장되어있으므로 조회건수 0
 ...
 SELECT table_name, tablespace_name,/* CONCAT('start','end' ).
    ...
    SUBSTR(문자열, 시작 인덱스, 종료인덱스) : 문자열의 시작인덱스 부터 ... 종료인덱스 까지 부분문자열
    시작인덱스는 1부터 (*java의 경우는 0부터)
    
    LENGTH(문자열) : 문자열의 길이를 반환
    
    INSTR(문자열, 찾을 문자열, [검색시작 인덱스]) : 문자열에서 찾을 문자열이 존재하는지, 존재할 경우 찾을 문자열의 인덱스 (위치) 반환
    LPAD,RPAD(문자열,맞추고 싶은 전체 문자열 길이,패딩 문자열)
    
    TRIM(문자열):문자열의 앞뒤의 존재하는 공백을 제거, 문자열 중간에 있는 공백은 제거대상이 아님.
   ...
   SELECT SUBSTR('Hello, World', 1,5) sub,
            ...
            INSTR('Hello, World', 'o') ins,
             ...
             INSTR('Hello, World', 'o', INSTR('Hello, World', 'o')+1) ins3, /*<-심화된버전 메모에적자 꺄르륵*/
             ...
             TRIM(' HELLO     ') tr,
             TRIM('H' FROM 'HEllo') tr2 (문자열 앞뒤로 특정문자 제거)
             FROM dual;
          
          
          NUMBER 관력함수
          ROUND(숫자, [반올림 위치-default 0]) : 반올림
          ROUND(105.54, 1): 소수점 첫번째 자리까지 결과를 생성==> 소수점 두번쨰 자리에서 반올림
          :105.5
...
SELECT MOD(10, 3), sal, MODE(sal, 1000)
FROM dual;

날짜 관련 함수

STSDATE : 사용중인 오라클 데이터베이스 서버의 현재 시간, 날짜를 반환한다
        함수이지만 인자가 없는 함수
        (인자가 없을 경우 JAVA : 메소드()
                        SQL : 함수명);
...
FROM dual;
 ...
 ADD_MONTHS(DATE1, 가감할 개월수) : DATE1로 부터 두번쨰 입력된 개월수 만큼 가감한 DATE
 오늘 날짜로부터 5개월 뒤 날짜 
 SELECT ADD_MONTHS(SYSDATE,5) dt1,
  ADD_MONTHS(SYSDATE,-5) dt2
 FROM dual;
 
 NEXT_DAY(date1, 주간일자-1~7) date이후 등장하는 첫번쨰 주간일자의 날짜를 반환
